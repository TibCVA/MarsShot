...
def main():
    ...
    while True:
        try:
            ...
            # 1) auto_select => 22h55
            if h==22 and m==55 and not state.get("did_auto_select_today",False):
                run_auto_select_once_per_day(state)

            # 2) daily => 23h05
            if h==23 and m==5 and not state.get("did_daily_update_today",False):
                logging.info("[MAIN] 23h05 => daily_update_live.")
