...
import sys
import pandas as pd  # ← utilisé plus bas pour écrire un CSV vide

from modules.trade_executor import TradeExecutor
from modules.positions_store import load_state, save_state
from modules.risk_manager import intraday_check_real

# === MARK A ───────── blocs utilitaires « top 60 performers » =============== #
try:
    from auto_select_tokens import (
        get_24h_change,
        get_kline_change,
        compute_token_score,
    )
except ImportError:
    # Si l'import direct échoue (cas très rare), on fournit des stubs neutres.
    def get_24h_change(*args, **kw):
        return 0.0

    def get_kline_change(*args, **kw):
        return 0.0

    def compute_token_score(p24, p7, p30=0.0):
        return -1e9


def pick_top_performers(client, token_list, top_n: int = 60):
    """
    Calcule 0.8*perf 7j + 0.2*perf 24h pour chaque token de token_list
    et renvoie ≤ top_n tokens au score décroissant.
    """
    scored = []
    for tok in token_list:
        pair = f"{tok.upper()}USDC"
        try:
            p24 = get_24h_change(client, pair)
            p7 = get_kline_change(client, pair, days=7)
            score = compute_token_score(p24, p7, 0.0)  # p30 non utilisé
        except Exception:
            score = -1e9
...
def load_probabilities_csv(csv_path=DAILY_PROBABILITIES_CSV_PATH):
    ...
    if not os.path.exists(csv_path):
        logger.warning(
            f"Fichier de probabilités {csv_path} introuvable => retour de {{}}"
        )
        return {}
    try:
        ...
        if df.empty:
            logger.warning(
                f"Fichier de probabilités {csv_path} est vide => retour de {{}}"
            )
            return {}
        prob_map = {}
        ...
        logger.warning(
            f"Fichier de probabilités {csv_path} est vide ou mal formaté (EmptyDataError) => retour de {{}}"
        )
        return {}
    except Exception as e:
        logger.error(
            f"Erreur lors de la lecture de {csv_path}: {e}", exc_info=True
        )
        return {}
...
def run_auto_select_once_per_day(state_unused):
    """... (fonction inchangée) ..."""
    # ── le contenu complet d'origine est conservé, pas de modification ──
    # (début)
    logger.info(
...
def daily_update_live(state, bexec):
    ...
    try:
        ...
        cfg_extended_tokens = config.get(
            "extended_tokens_daily", "Non présente ou vide"
        )
        logger.info(
            f"'extended_tokens_daily' lu depuis {CONFIG_FILE_PATH}: {str(cfg_extended_tokens)[:200]}..."
        )
    except Exception as e:
        logger.error(
            f"Erreur lors de la lecture de {CONFIG_FILE_PATH}: {e}", exc_info=True
        )
        return
    ...
    if auto_selected_tokens_from_script is not None:
        ...
        logger.info(
            f"Utilisation de la liste de tokens retournée par auto_select_tokens.py ({len(auto_selected_tokens)} tokens)."
        )
    else:
        logger.warning(
            "auto_select_tokens.py n'a pas retourné de liste de tokens. Tentative de lecture depuis config.yaml (extended_tokens_daily)."
        )
        auto_selected_tokens = config.get("extended_tokens_daily", [])
        if not isinstance(auto_selected_tokens, list):
            logger.warning(
                "'extended_tokens_daily' dans config (fallback) n'est pas une liste. Utilisation d'une liste vide."
            )
            auto_selected_tokens = []

    # ======================================================================
    # MARK B ── Construction des listes : top60 + positions
    # ======================================================================
    manual_tokens = config.get("tokens_daily", [])
    if not isinstance(manual_tokens, list):
        manual_tokens = []

    system_positions = list(state.get("positions_meta", {}).keys())

    # 1) calcul TOP 60 parmi les 178 tokens « manual »
    try:
        tokens_top60 = pick_top_performers(
            bexec.client, manual_tokens, top_n=60
        )
    except Exception as e_perf:
        logger.error(f"[pick_top_performers] fallback : {e_perf}")
        tokens_top60 = manual_tokens[:60]  # secours

    logger.info(f"TOP60 perf : {tokens_top60[:10]} … ({len(tokens_top60)})")

    # 2) tokens nécessaires au calcul des features (top60 + positions détenues)
    final_token_list_for_fetcher = sorted(
        set(tokens_top60).union(system_positions)
    )

    # 3) cette liste (top60) est la seule autorisée pour les achats
    tokens_for_buy_pool = tokens_top60
    # ======================================================================

    logger.info(
        f"Tokens from auto_select_script (direct return or fallback from config): {len(auto_selected_tokens)} - Aperçu: {str(auto_selected_tokens[:10]) if auto_selected_tokens else '[]'}"
    )
    logger.info(f"Tokens from manual list (config:tokens_daily): {manual_tokens}")
    logger.info(
        f"Tokens from current positions (state:positions_meta): {system_positions}"
    )
    logger.info(
        f"Liste finale combinée pour data_fetcher ({len(final_token_list_for_fetcher)} tokens) - Aperçu: {str(final_token_list_for_fetcher[:10]) if final_token_list_for_fetcher else '[]'}"
    )

    if not final_token_list_for_fetcher:
        logger.warning(
            "La liste finale de tokens pour data_fetcher est vide. Arrêt du daily_update."
        )
        try:
            pd.DataFrame().to_csv(DAILY_INFERENCE_CSV_PATH, index=False)
            logger.info(
                f"Fichier {os.path.basename(DAILY_INFERENCE_CSV_PATH)} vide créé."
            )
        except Exception as e_csv:
            logger.error(
                f"Erreur lors de la création du fichier CSV vide {os.path.basename(DAILY_INFERENCE_CSV_PATH)}: {e_csv}"
            )
        return
    ...
    logger.info(
        f"{os.path.basename(CONFIG_TEMP_FILE_PATH)} créé avec {len(final_token_list_for_fetcher)} tokens dans extended_tokens_daily."
    )

    # --- le reste de daily_update_live est inchangé ----------------------- #
    # (stratégie, data_fetcher, ml_decision, SELL, BUY)
    ...
    try:
        ...
        logger.error(
            f"Valeur invalide pour big_gain_exception_pct: {strat.get('big_gain_exception_pct')}. Utilisation de 3.0."
        )
        big_gain_pct = 3.0
    ...
    try:
        logger.info(
            f"Exécution de {os.path.basename(DATA_FETCHER_SCRIPT_PATH)} avec {CONFIG_TEMP_FILE_PATH}"
        )
        ...
        if process_df.stdout:
            logger.info(f"data_fetcher stdout:\n{process_df.stdout.strip()}")
        if process_df.stderr:
            logger.warning(f"data_fetcher stderr:\n{process_df.stderr.strip()}")
    except subprocess.CalledProcessError as e:
        logger.error(
            f"{os.path.basename(DATA_FETCHER_SCRIPT_PATH)} a échoué (code: {e.returncode})."
        )
        if hasattr(e, "stdout") and e.stdout:
            logger.error(f"data_fetcher stdout de l'erreur:\n{e.stdout.strip()}")
        if hasattr(e, "stderr") and e.stderr:
            logger.error(f"data_fetcher stderr de l'erreur:\n{e.stderr.strip()}")
        return
    except Exception as e:
        logger.error(
            f"Exception lors de l'appel à {os.path.basename(DATA_FETCHER_SCRIPT_PATH)}: {e}",
            exc_info=True,
        )
        return
    ...
    try:
        ...
        process_ml = subprocess.run(
            [python_executable, ML_DECISION_SCRIPT_PATH],
            check=True,
            capture_output=True,
            text=True,
            cwd=PROJECT_ROOT,
        ...
        if process_ml.stdout:
            logger.info(f"ml_decision stdout:\n{process_ml.stdout.strip()}")
        if process_ml.stderr:
            logger.warning(f"ml_decision stderr:\n{process_ml.stderr.strip()}")
    except subprocess.CalledProcessError as e:
        logger.error(
            f"{os.path.basename(ML_DECISION_SCRIPT_PATH)} a échoué (code: {e.returncode})."
        )
        if hasattr(e, "stdout") and e.stdout:
            logger.error(f"ml_decision stdout de l'erreur:\n{e.stdout.strip()}")
        if hasattr(e, "stderr") and e.stderr:
            logger.error(f"ml_decision stderr de l'erreur:\n{e.stderr.strip()}")
        return
    except Exception as e:
        logger.error(
            f"Exception lors de l'appel à {os.path.basename(ML_DECISION_SCRIPT_PATH)}: {e}",
            exc_info=True,
        )
        return
    ...
    logger.info(
        f"Probabilités chargées pour {len(prob_map)} tokens. Liste de fetcher: {len(final_token_list_for_fetcher)} tokens."
    )
    ...
    # (boucle SELL originale conservée)
    ...
    for asset, real_qty in list(holdings.items()):
        ...
        if current_px <= 0:
            logger.warning(
                f"Prix invalide ou nul ({current_px}) pour {asset} lors de la vérification de vente."
            )
            continue
        ...
        logger.info(
            f"Vérification VENTE pour {asset}: Val={val_in_usd:.2f}, Qty={real_qty}, Px={current_px:.4f}, Prob={prob if prob is not None else 'N/A'}"
        )
        ...
        if val_in_usd >= MIN_VALUE_TO_SELL and prob < sell_threshold:
            ...
            if entry_px > 0:
                ...
                if ratio >= big_gain_pct:
                    if not did_skip:
                        ...
                        logger.info(
                            f"SKIP VENTE (BIG GAIN EXCEPTION) pour {asset}: ratio={ratio:.2f} >= {big_gain_pct:.2f}. Marqué pour ne plus skipper."
                        )
                        perform_sell = False
                    else:
                        logger.info(
                            f"VENTE AUTORISÉE (BIG GAIN EXCEPTION DÉJÀ UTILISÉE) pour {asset}: ratio={ratio:.2f}."
                        )
            if perform_sell:
                logger.info(
                    f"Condition de VENTE REMPLIE pour {asset} (Prob: {prob:.2f} < {sell_threshold}, Val: {val_in_usd:.2f} >= {MIN_VALUE_TO_SELL}). Vente en cours..."
                )
            ...
            logger.debug(
                f"Skip vente {asset}. Conditions non remplies (Val: {val_in_usd:.2f}, Prob: {prob}, SellThr: {sell_threshold})."
            )

    if assets_sold_this_cycle:
        save_state(state)
        logger.info(
            f"État sauvegardé après phase de VENTE. Tokens vendus: {assets_sold_this_cycle}"
        )
    ...
    try:
        ...
        logger.error(
            f"get_account après attente (post-vente) a échoué: {e}", exc_info=True
        )
        return
    ...
    logger.info(
        f"Après attente (post-vente) => Holdings: {new_holdings}, USDC: {new_USDC_balance:.2f}"
    )

    # ⟱⟱⟱  SEULE LIGNE MODIFIÉE : on limite la recherche aux tokens du pool BUY  ⟱⟱⟱
    buy_candidates_source_list = tokens_for_buy_pool
    # ⟰⟰⟰  auparavant c'était : buy_candidates_source_list = final_token_list_for_fetcher
    ...
    for sym in buy_candidates_source_list:
        ...
        if p is None or p < buy_threshold:
            if p is None:
                logger.debug(f"SKIP ACHAT {sym}: Probabilité non trouvée.")
            else:
                logger.debug(
                    f"SKIP ACHAT {sym}: Probabilité {p:.2f} < seuil d'achat {buy_threshold:.2f}."
                )
            continue
        ...
        if current_quantity_held > 0:
            ...
            if price_for_value_check > 0:
                ...
                if value_held > MAX_VALUE_TO_SKIP_BUY:
                    logger.info(
                        f"SKIP ACHAT {sym}: Déjà en portefeuille avec valeur {value_held:.2f} USDC (> {MAX_VALUE_TO_SKIP_BUY} USDC)."
                    )
                    continue
            else:
                logger.warning(
                    f"Vérification ACHAT {sym}: Impossible de récupérer le prix pour la valeur détenue. Achat autorisé si prob OK."
                )
    ...
    if top3_buy_candidates and new_USDC_balance > 10:
        ...
        logger.info(
            f"Allocation de {usdc_to_allocate_total:.2f} USDC pour {num_buys_to_make} token(s)."
        )
        ...
        for i, (sym, p_val) in enumerate(top3_buy_candidates, start=1):
            ...
            if usdc_to_allocate_total < 10 or remaining_tokens_to_buy == 0:
                logger.info(
                    "Reliquat USDC < 10 ou plus de tokens à acheter dans ce cycle. Arrêt des achats."
                )
                break
            ...
            if usdc_per_buy < 5:
                logger.info(
                    f"Allocation par achat pour {sym} ({usdc_per_buy:.2f} USDC) trop faible. Arrêt des achats."
                )
                break
            ...
            if qty_bought > 0 and cost_of_buy > 0:
                logger.info(
                    f"ACHAT RÉUSSI {sym}: {qty_bought:.4f} pour {cost_of_buy:.2f} USDC @ {price_bought:.4f}"
                )
                state.setdefault("positions_meta", {})[sym] = {
                    "entry_px": price_bought,
                    "did_skip_sell_once": False,
                    "partial_sold": False,
                    "max_price": price_bought,
                }
                usdc_to_allocate_total -= cost_of_buy
                assets_bought_this_cycle.append(sym)
            else:
                logger.warning(
                    f"ACHAT ÉCHOUÉ/SKIPPÉ pour {sym}. Achat non effectué ou quantité/coût nul."
                )

        if assets_bought_this_cycle:
            save_state(state)
            logger.info(
                f"État sauvegardé après phase d'ACHAT. Tokens achetés: {assets_bought_this_cycle}"
            )
    else:
        if not top3_buy_candidates:
            logger.info("Aucun candidat d'achat trouvé répondant aux critères.")
        if new_USDC_balance <= 10:
            logger.info(
                f"Solde USDC ({new_USDC_balance:.2f}) insuffisant pour initier des achats."
            )
...
# ─────────────────────────────────────────────────────────────────────────── #
def main():
    # (contenu strictement inchangé, hors marqueur d'heure par défaut)
    if not os.path.exists(CONFIG_FILE_PATH):
        print(
            f"[ERREUR CRITIQUE] {CONFIG_FILE_PATH} introuvable. Le bot ne peut pas démarrer."
        )
    ...
    try:
        ...
        print(
            f"[ERREUR CRITIQUE] Impossible de lire ou parser {CONFIG_FILE_PATH}: {e}. Arrêt."
        )
        logging.basicConfig(level=logging.CRITICAL)
        logging.critical(
            f"[MAIN] Impossible de lire ou parser {CONFIG_FILE_PATH}: {e}. Arrêt."
        )
        return
    ...
    if log_dir and not os.path.exists(log_dir):
        try:
            ...
            print(
                f"Erreur création répertoire log {log_dir}: {e}. Tentative d'écriture dans le répertoire courant."
            )
    ...
    logger.info(
        f"Boucle principale démarrée. Mise à jour quotidienne prévue à {DAILY_UPDATE_HOUR_UTC:02d}:{DAILY_UPDATE_MINUTE_UTC:02d} UTC."
    )
    ...
    while True:
        try:
            ...
            if last_update_timestamp > 0:
                last_update_date_utc = datetime.datetime.fromtimestamp(
                    last_update_timestamp, tz=pytz.utc
                ).date()

            if state.get("did_daily_update_today", False):
                if last_update_date_utc is None or current_date_utc > last_update_date_utc:
                    logger.info(
                        f"Réinitialisation du flag 'did_daily_update_today' car nouveau jour UTC ({current_date_utc}) par rapport à la dernière màj ({last_update_date_utc})."
                    )
                    state["did_daily_update_today"] = False
                    save_state(state)

            if (
                now_utc.hour == DAILY_UPDATE_HOUR_UTC
                and now_utc.minute == DAILY_UPDATE_MINUTE_UTC
                and not state.get("did_daily_update_today", False)
            ):

                logger.info(
                    f"Déclenchement de daily_update_live (heure planifiée: {now_utc.strftime('%Y-%m-%d %H:%M:%S %Z')})."
                )
                ...
                save_state(state)
                logger.info(
                    "daily_update_live terminé et flag 'did_daily_update_today' positionné."
                )
            ...
            if current_time_for_check - last_risk_check_ts >= check_interval:
                logger.info(
                    f"Exécution de intraday_check_real() (dernier check il y a {current_time_for_check - last_risk_check_ts:.0f}s)."
                )
