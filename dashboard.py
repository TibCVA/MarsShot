...
try:
    ...
    from main import configure_main_logging 
    from modules.trade_executor import TradeExecutor
    main_module_imported_successfully = True
except ImportError as e:
    dashboard_logger.error(f"Erreur import modules main/TradeExecutor pour force_daily_update: {e}.")
    def main_daily_update_live(state, bexec): raise RuntimeError("Dépendances main.py manquantes.")
    def load_state(): return {}
    def configure_main_logging(s=None): pass
    class TradeExecutor: __init__ = lambda self, api_key, api_secret: None
...
# REMETTEZ VOTRE TEMPLATE HTML COMPLET ICI
TEMPLATE_HTML = r"""
<!DOCTYPE html>
<html lang="en">
<head>
...
function forceDailyUpdate(){
  if(confirm("Forcer la mise à jour quotidienne maintenant ? Cela peut prendre plusieurs minutes.")) {
    ...
    .then(response => {
        if (!response.ok) {
            // Essayer de parser le JSON même pour les erreurs pour obtenir le message du serveur
...
def force_daily_update(pwd):
    # ... (corps de la fonction inchangé, celui que j'ai fourni précédemment) ...
    if pwd != SECRET_PWD: return jsonify({"message": "Accès Interdit"}), 403
    ...
    try:
        if not os.path.exists(CONFIG_FILE_PATH_DASH):
            ...
            dashboard_logger.error(msg); return jsonify({"message": msg}), 500
        with open(CONFIG_FILE_PATH_DASH, "r", encoding="utf-8") as f: config_main = yaml.safe_load(f)
        main_log_settings = config_main.get("logging", {})
        configure_main_logging(main_log_settings) 
        dashboard_logger.info("Logging pour 'main_bot_logic' reconfiguré pour cet appel.")
        state = load_state()
        binance_api_config = config_main.get("binance_api", {}); api_key = binance_api_config.get("api_key"); api_secret = binance_api_config.get("api_secret")
