...
# On veut lire config.yaml dans la racine => ../config.yaml
CONFIG_FILE = os.path.join(CURRENT_DIR, "..", "config.yaml")
# Ecrire daily_inference_data.csv dans la racine => ../daily_inference_data.csv
...
def fetch_lunar_data_inference(symbol, lookback_days=365):
    end_date = datetime.utcnow()
    start_date = end_date - timedelta(days=lookback_days)
    start_ts = int(start_date.timestamp())
    end_ts   = int(end_date.timestamp())

    url = "https://lunarcrush.com/api4/public/coins/{}/time-series/v2".format(symbol)
    params= {
      "key": LUNAR_KEY,
      "bucket":"day",
      "start": start_ts,
...
def main():
    ...
    for i, sym in enumerate(TOKENS_DAILY, start=1):
        ...
        needed_cols = [
            "date","symbol","delta_close_1d","delta_close_3d","delta_vol_1d","delta_vol_3d",
        ...
        if not merged.empty:
            ...
            logging.info(f"[TOKEN OK] {sym} => shape={merged.shape}")
        else:
