...
class TradeExecutor:
    ...
    def sell_all(self, asset, qty):
        ...
        # Avant l'ordre, on arrondit la qty selon stepSize
    ...
    def buy(self, asset, usdt_amount):
        ...
        try:
            ...
            # Double check => on calcule cost potentielle
            cost_est = px * adj_qty
            # On peut vérifier minNotional => si cost < minNotional => skip
            # => la fonction adjust_quantity_lot_size le gère déjà, ou on le re-check
    ...
    def adjust_quantity_lot_size(self, symbol, raw_qty):
        ...
        Ajuste la quantité 'raw_qty' pour respecter:
          ...
          - minQty (LOT_SIZE)
          - minNotional
        Retourne la nouvelle quantité. Si c'est 0 => ordre trop petit => skip.
        """
        try:
            info = self.client.get_symbol_info(symbol)
            # Ex: info["filters"] contient LOT_SIZE, MIN_NOTIONAL, etc.
            ...
            min_qty   = float(lot_size_filter["minQty"])   if lot_size_filter else 0.00000001
            min_notional= float(min_notional_filter["minNotional"]) if min_notional_filter else 10.0

            # On arrondit la qty au multiple step_size
            # ex: real_qty = floor(raw_qty / step_size) * step_size
            adj_qty = (raw_qty // step_size) * step_size  # integer division trick

            # Vérifie si adj_qty < min_qty => skip
            if adj_qty < min_qty:
                return 0.0

            # Vérifie minNotional => price * adj_qty >= min_notional
            # => on récupère le prix
